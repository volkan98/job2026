...
async function updateCampaignStats(sb: any, campaignId: string) {
  const counts = await Promise.all([
    sb.from("campaign_queue").select("id", { count: "exact", head: true }).eq(
      "campaign_id",
      campaignId,
    ),
    sb.from("campaign_queue").select("id", { count: "exact", head: true }).eq(
...
async function getHourlySentCount(sb: any, userId: string): Promise<number> {
  ...
  const { count } = await sb.from("sent_emails").select("id", {
    count: "exact",
    head: true,
  })
...
async function sendViaGmail(
  ...
  const encodedMessage = btoa(unescape(encodeURIComponent(rawMessage))).replace(
    /\+/g,
    "-",
  ).replace(/\//g, "_").replace(/=+$/, "");
...
async function fetchCVAttachment(sb: any, filePath: string) {
  try {
    const { data, error } = await sb.storage.from("cv-files").download(
      filePath,
    );
...
async function processCampaign(sb: any, campaign: any) {
  ...
  console.log(
    `Processing campaign ${campaignId} for user ${userId}, status: ${campaign.status}`,
  );
  ...
  if (campaign.status === "paused" && campaign.resume_at) {
    ...
    await logEvent(
      ...
      userId,
      "auto_resume",
      "Campagna ripresa automaticamente dopo pausa anti-spam",
    );
  }

  // Check target reached
  const { count: sentCount } = await sb.from("campaign_queue").select("id", {
    count: "exact",
    head: true,
  })
  ...
  if ((sentCount || 0) >= campaign.target_total) {
    ...
    await logEvent(
      ...
      userId,
      "target_completed",
      `Target di ${campaign.target_total} email raggiunto!`,
    );
  ...
  if (dailySent >= 80) {
    ...
    await sb.from("auto_campaigns").update({
      status: "paused",
      pause_reason: "Limite giornaliero (80 email) raggiunto",
      resume_at: resumeAt,
      updated_at: new Date().toISOString(),
    }).eq("id", campaignId);
    await logEvent(
      ...
      userId,
      "paused_rate_limit",
      "Limite giornaliero raggiunto, pausa di 1 ora",
    );
    return;
  }
  ...
  const { data: tokenData } = await sb.from("email_oauth_tokens").select("*")
    .eq("user_id", userId).eq("provider", "gmail").single();
  if (!tokenData) {
    await sb.from("auto_campaigns").update({
      status: "paused",
      pause_reason: "Gmail non connesso",
      updated_at: new Date().toISOString(),
    }).eq("id", campaignId);
    await logEvent(
      sb,
      ...
      userId,
      "error",
      "Gmail non connesso. Connetti Gmail per continuare.",
    );
    return;
  }
  ...
  if (
    ...
    new Date(tokenData.token_expires_at) < new Date()
  ) {
    try {
      ...
      await sb.from("auto_campaigns").update({
        status: "paused",
        pause_reason: "Token Gmail scaduto, riconnetti",
        updated_at: new Date().toISOString(),
      }).eq("id", campaignId);
      await logEvent(
        sb,
        ...
        userId,
        "error",
        "Impossibile rinnovare token Gmail",
      );
      return;
    }
  }
  ...
  const { count: pendingCount } = await sb.from("campaign_queue").select("id", {
    count: "exact",
    head: true,
  })
  ...
  if ((pendingCount || 0) === 0) {
    ...
    if (campaign.current_search_cycle > campaign.max_search_cycles) {
      await sb.from("auto_campaigns").update({
        status: "completed",
        completed_at: new Date().toISOString(),
        pause_reason: "Ricerche esaurite",
        updated_at: new Date().toISOString(),
      }).eq("id", campaignId);
      await logEvent(
        sb,
        ...
        userId,
        "search_exhausted",
        `Nessun nuovo risultato dopo ${campaign.max_search_cycles} cicli di ricerca`,
      );
      await updateCampaignStats(sb, campaignId);
      return;
    }
    ...
    await logEvent(
      ...
      userId,
      "search_started",
      `Ciclo di ricerca #${campaign.current_search_cycle}`,
    );
    ...
    try {
      ...
      if (!searchResult.success || !searchResult.data?.length) {
        ...
        await logEvent(
          ...
          userId,
          "search_empty",
          "Nessun nuovo risultato trovato in questo ciclo",
        );
        await updateCampaignStats(sb, campaignId);
        return;
      }
      ...
      for (const company of searchResult.data) {
        ...
        const { count: existsCount } = await sb.from("campaign_queue").select(
          "id",
          { count: "exact", head: true },
        )
          .eq("campaign_id", campaignId).eq(
            "company_email",
      ...
      await logEvent(
        ...
        userId,
        "search_completed",
        `Trovate ${added} nuove aziende valide`,
        { total_results: searchResult.data.length, added },
      );
      await updateCampaignStats(sb, campaignId);
    } catch (err: any) {
  ...
  const { data: pendingItems } = await sb.from("campaign_queue").select("*")
    ...
    .order("confidence_score", { ascending: false }).limit(
      EMAILS_PER_INVOCATION,
    );

  if (pendingItems?.length > 0) {
    // Get user profile for CV data
    const { data: profile } = await sb.from("user_profiles").select("*").eq(
      "user_id",
      userId,
    ).single();
    if (!profile) {
    ...
    for (const item of pendingItems) {
      try {
        const genResponse = await fetch(
          `${SUPABASE_URL}/functions/v1/ai-generate-email`,
          {
            method: "POST",
            headers: fnHeaders,
            body: JSON.stringify({
        ...
        if (!genResult.success || !genResult.data) {
          throw new Error(genResult.error || "Gen failed");
        }
        ...
        if (retryCount >= item.max_retries) {
          ...
          await sb.from("campaign_queue").update({
            retry_count: retryCount,
            next_retry_at: new Date(Date.now() + retryCount * 60000)
              .toISOString(),
            error_message: err.message,
          }).eq("id", item.id);
        }
  ...
  for (const item of readyItems) {
    try {
      ...
      await logEvent(
        ...
        userId,
        "email_sent",
        `Email inviata a ${item.company_name} (${item.company_email})`,
      );
      ...
      if (isRateLimit) {
        ...
        await logEvent(
          ...
          userId,
          "paused_rate_limit",
          "Gmail rate limit, pausa automatica",
        );
        break;
      }
      ...
      if (isTransient && retryCount < item.max_retries) {
        await sb.from("campaign_queue").update({
          retry_count: retryCount,
          next_retry_at: new Date(Date.now() + retryCount * 2 * 60000)
            .toISOString(),
          error_message: err.message,
        }).eq("id", item.id);
      } else {
        ...
        await logEvent(
          ...
          userId,
          "send_failed",
          `Invio fallito per ${item.company_name}: ${err.message}`,
        );
      }
    }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (targetCampaignId) {
      ...
      query = query.or(
        `status.eq.running,and(status.eq.paused,resume_at.lte.${
    ...
    for (const campaign of campaigns.slice(0, 2)) {
      if (
        ...
        (campaign.status === "paused" && campaign.resume_at)
      ) {
